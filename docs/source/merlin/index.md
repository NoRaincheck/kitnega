---
title: Merlin — Random-Split Forests with TreeSHAP
---

# Merlin

Machine learning library using random-split trees. Stdlib + `lib.array` only. All three forest types share the same underlying `_Node` tree structure and are unified under a single `RandomForest(task=…, split=…)` class.
...
- **No true Mondrian properties**: the trees are not guaranteed to have the theoretical properties of Mondrian processes.
...
merlin implements the polynomial-time TreeSHAP algorithm (Lundberg et al. 2017) using a per-leaf subset enumeration approach. Each leaf categorises path features into _toward_ (x follows the same direction) and _away_ (x goes opposite), and computes contributions using Shapley kernel weights that account for both path and non-path features. Repeated features on a path are collapsed automatically.
