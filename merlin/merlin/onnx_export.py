...
def to_onnx(forest, input_name="X", label_name="label", proba_name="probabilities",
    ...
    if forest.task == "classifier":
        graph_nodes, graph_outputs, initializers = _build_classifier_graph(
            cls_nodes, cls_leaf_attrs, forest, input_name,
            label_name, proba_name,
        )
    else:
        graph_nodes, graph_outputs, initializers = _build_regressor_graph(
            reg_nodes, reg_leaf_attrs, input_name, output_name,
        )

    graph = helper.make_graph(graph_nodes, "merlin_forest", inputs,
    ...
    model = helper.make_model(graph, opset_imports=opset_imports,
                              producer_name="merlin", producer_version="0.1.0")
    return model
...
def _build_classifier_graph(nodes, leaf_attrs, forest, input_name,
                             label_name, proba_name):
    ...
    topk_node = helper.make_node(
        "TopK", [proba_name, "_k_val"], ["_top_values", "_top_indices"],
        axis=1, largest=1,
    )
    ...
    reshape_node = helper.make_node(
        "Reshape", ["_top_indices", "_shape_val"], ["_flat_indices"],
    )
    cls_tensor = numpy_helper.from_array(
        np.array(classlabels, dtype=np.int64), "classlabels",
    )
    gather_node = helper.make_node(
        "Gather", ["classlabels", "_flat_indices"], [label_name], axis=0,
    )
    ...
    graph_outputs = [
        ...
        helper.make_tensor_value_info(proba_name, TensorProto.FLOAT,
                                      [None, n_classes]),
    ]
    ...
    def _collect(node):
        ...
        if node.is_leaf:
            ...
            if forest.task == "classifier":
                ...
                leaf_attrs.append((tree_idx, nid, 0,
                                   float(node.prediction if node.prediction is not None else 0.0)))
        else:
            nodes.append({
                "tree_id": tree_idx,
                "node_id": nid,
