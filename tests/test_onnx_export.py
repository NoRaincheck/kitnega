...
def _split_nodes(node):
    ...
    return [(node.feature, node.threshold)] \
        + _split_nodes(node.left) \
        + _split_nodes(node.right)
...
class TestStructuralCorrectness:
    ...
    def test_node_count_per_tree(self):
        ...
        for t_idx, tree in enumerate(clf.trees_):
            ...
            onnx_count = sum(
                1 for nid, tid in zip(onnx_nodes["nodes_nodeids"], onnx_nodes["nodes_treeids"])
                if tid == t_idx
            )
            assert merlin_count == onnx_count, (
                f"Tree {t_idx}: merlin has {merlin_count} nodes, ONNX has {onnx_count}"
            )

    def test_thresholds_and_features_match(self):
        X = array([[1.0, 2.0], [2.0, 3.0], [3.0, 4.0], [4.0, 5.0]])
        y = array([0.0, 0.0, 1.0, 1.0])
        clf = RandomForest(n_estimators=2, task="classifier", split="best",
        ...
        for t_idx, tree in enumerate(clf.trees_):
            ...
            assert len(merlin_splits) == len(onnx_splits), (
                f"Tree {t_idx}: {len(merlin_splits)} merlin splits, "
                f"{len(onnx_splits)} ONNX splits"
            )
...
class TestClassifierPredictions:
    def test_binary_classification(self):
        X = array([[1.0, 2.0], [2.0, 3.0], [3.0, 4.0], [4.0, 5.0]])
        y = array([0.0, 0.0, 1.0, 1.0])
        clf = RandomForest(n_estimators=5, task="classifier", split="best",
                           max_depth=3, random_state=42)
        clf.fit(X, y)
    ...
    def test_mondrian_classifier(self):
        X = array([[1.0, 2.0], [2.0, 3.0], [3.0, 4.0], [4.0, 5.0]])
        y = array([0.0, 0.0, 1.0, 1.0])
        clf = RandomForest(n_estimators=5, task="classifier", split="mondrian",
                           random_state=42)
        clf.fit(X, y)
        onnx_label, onnx_proba = _roundtrip(clf, _to_numpy(X))
        n_trees = len(clf.trees_)
    ...
    def test_no_bootstrap(self):
        ...
        y = array([0.0, 0.0, 1.0, 1.0])
        clf = RandomForest(n_estimators=5, task="classifier", split="best",
                           max_depth=3, bootstrap=False, random_state=42)
        clf.fit(X, y)
        onnx_label, onnx_proba = _roundtrip(clf, _to_numpy(X))
        n_trees = len(clf.trees_)
        merlin_label = list(clf.predict(X).flat)
...
class TestRegressorPredictions:
    def test_regression(self):
        ...
        reg = RandomForest(n_estimators=5, task="regressor", split="best",
                           max_depth=3, random_state=42)
        reg.fit(X, y)
    ...
    def test_mondrian_regressor(self):
        X = array([[1.0], [2.0], [3.0], [4.0]])
        y = array([1.0, 2.0, 3.0, 4.0])
        reg = RandomForest(n_estimators=5, task="regressor", split="mondrian",
                           random_state=42)
        reg.fit(X, y)
        onnx_pred = _roundtrip(reg, _to_numpy(X))[0]
        merlin_pred = list(reg.predict(X).flat)
...
def _assert_labels_match(onnx_label, merlin_label, n_trees):
    ...
    mismatches = sum(
        1 for o, m in zip(onnx_label, merlin_float) if abs(float(o) - float(m)) > 0.5
    )
    total = len(merlin_float)
    assert mismatches <= max(1, total // 10), (
        f"ONNX labels {list(onnx_label)} != merlin labels {merlin_float} "
...
def _assert_probas_close(onnx_proba, merlin_proba, n_trees):
    ...
    for i in range(len(onnx_proba)):
        for j in range(len(onnx_proba[i])):
            ...
            assert diff < 1e-5, (
                f"proba mismatch at ({i},{j}): "
                f"onnx={onnx_proba[i][j]}, expected={expected}"
            )
